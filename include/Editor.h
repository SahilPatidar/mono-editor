...
public:
  ...
  inline void put(Line line) noexcept { Items.push_back(line); }

  inline void drop_back() noexcept { Items.pop_back(); }
...
public:
  ...
  void backSpace(size_t StarCursor, size_t EndCursor);
...
class Editor {
public:
  Editor(const char *_filePath) : filePath(_filePath) {}
  ~Editor() {}
  ...
  void RenderChar(TextRenderer &R, char c, Vec2 &Pos, TTF_Font *Font,
                  SDL_Color &Color);
  ...
  void CopyClip();
  void PasteClip();
  void saveBufToSource() noexcept { Buf.write_file(filePath); }
  ...
  void setSelectMark() noexcept {
    ...
    MarkedRow = Selecting ? CursorRow : -1;
  }
  inline int getSelectMark() const noexcept { return SelectedMark; }

  inline void CalcWindowStartPos() noexcept {
    static int Reduceable = 0;
    if (CursorRow > WindowBlock.RowEnd - 1) {
      ++WindowBlock.RowStart;
      WindowBlock.RowEnd = CursorRow + 1;
      ++Reduceable;
    } else if (Reduceable) {
      if (CursorRow < WindowBlock.RowStart) {
        WindowBlock.RowStart = CursorRow;
        WindowBlock.RowEnd = CursorRow + (SCREEN_HEIGHT / (FONT_HEIGHT * FONT_SCALE));
        --Reduceable;
      }
    }
  }
  ...
  inline void CalcWindowColStartPos() noexcept {
    ...
    if (CursorCol > WindowBlock.ColEnd - 1) {
      ...
      WindowBlock.ColEnd = CursorCol + 1;
      ++Reduceable;
    } else if (Reduceable) {
      if (CursorCol < WindowBlock.ColStart) {
        WindowBlock.ColStart = CursorCol;
        WindowBlock.ColEnd = CursorCol + (SCREEN_WIDTH / (FONT_WIDTH * FONT_SCALE));
        --Reduceable;
      }
    }
  }
